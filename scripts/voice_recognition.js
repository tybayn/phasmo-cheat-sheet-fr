...
function parse_speech(vtext) {
    ...
    if (vtext.startsWith('ghost speed') || vtext.startsWith('vitesse entité')) {
        ...
        var smallest_val = 100
        var vvalue = 0
        if (vtext.startsWith("not ") || vtext.startsWith("knot ") || vtext.startsWith('pas ')) {
            vtext = vtext.replace('not ', "").replace('knot ', "").replace('pas ', "").trim()
            vvalue = 0
        } else if (vtext.startsWith("undo ") || vtext.startsWith("undue ") || vtext.startsWith("on do ") || vtext.startsWith("on due ") || vtext.startsWith("clear") || vtext.startsWith("annuler")) {
            vtext = vtext.replace('undo ', "").replace('undue ', "").replace("on do ", "").replace("on due ", "").replace("clear ", "").replace("annuler ", "").trim()
            vvalue = 0
                ...
                vvalue = 2
            } else if (vtext.startsWith("hide ") || vtext.startsWith("remove ") || vtext.startsWith("cacher ") || vtext.startsWith("barrer ")) {
        ...
        } else if (vtext.startsWith('hunt sanity') || vtext.startsWith('sanity') || vtext.startsWith('santé mentale')) {
            document.getElementById("voice_recognition_status").className = null
            document.getElementById("voice_recognition_status").style.backgroundImage = "url(imgs/mic-recognized.png)"
            console.log("Recognized speed command")
            console.log(`Heard '${vtext}'`)
            ...
            reset_voice_status()
        } else if (vtext.startsWith('show tools') || vtext.startsWith('show filters') || vtext.startsWith('voir outils') || vtext.startsWith('voir filtres')) {
            document.getElementById("voice_recognition_status").className = null
            document.getElementById("voice_recognition_status").style.backgroundImage = "url(imgs/mic-recognized.png)"
