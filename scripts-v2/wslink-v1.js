...
function link_room() {
    ...
    ws.onopen = function (event) {
        ...
        document.getElementById("room_id_note").innerText = "STATUT: Connecté"
    ...
    ws.onerror = function (event) {
        document.getElementById("room_id_note").innerText = "ERREUR: N'a pas pu se connecter !"
        ...
        try {
            ...
            if (incoming_state.hasOwnProperty("action")) {
                ...
                if (incoming_state['action'].toUpperCase() == "CHANGE"){
                    document.getElementById("room_id_note").innerText = `STATUT: Connecté (${incoming_state['players']})`
}
            ...
            if (incoming_state.hasOwnProperty("error")) {
                ...
                document.getElementById("room_id_note").innerText = `ERREUR: ${incoming_state['error']}!`
                document.getElementById("settings_status").className = "error"
...
function link_link() {
    ...
    dlws.onopen = function (event) {
        ...
        document.getElementById("link_id_note").innerText = "STATUT: En attente du lien vers l'application de bureau"
        document.getElementById("dllink_status").className = "pending"
    }
    dlws.onerror = function (event) {
        document.getElementById("link_id_note").innerText = "ERREUR: N'a pas pu se connecter !"
    ...
    dlws.onmessage = function (event) {
        try {
                ...
                if (incoming_state['action'].toUpperCase() == "LINKED") {
                    ...
                    dlws_ping = setInterval(function(){
                        if (await_dlws_pong){
                            ...
                            document.getElementById("link_id_note").innerText = "ERREUR : lien perdu, connexion !"
                            document.getElementById("dllink_status").className = "error"
            ...
            if (incoming_state.hasOwnProperty("error")) {
                document.getElementById("link_id_note").innerText = `ERREUR: ${incoming_state['error']}!`
                document.getElementById("dllink_status").className = "error"
            }
...
function continue_session(){
    if(hasLink){
        ...
        polled = true
        $("#reset").html("En attendant les autres...")
        return false
    }
    return true
}
...
function disconnect_room(reset = false, has_status = false) {
    ...
    if (!reset){
        ...
        if(!has_status){
            document.getElementById("room_id_note").innerText = "STATUT: Non connecté"
...
function send_ghost_data_link(ghost){
    if(hasDLLink){
        ...
        data += document.getElementById(ghost).querySelector(".ghost_evidence").innerText.trim().replaceAll("\n",", ") + (ghost == "The Mimic" ? ", *Orbe" : "") + "\n"
        data += document.getElementById(ghost).querySelector(".ghost_behavior").innerText
        data = data.replace("On sait","\n<b>On sait:<b>\n")
        data = data.replace("Comportements","\n<b>Comportements:<b>\n")
        data = data.replace("Seuil de chasse","\n<b>Seuil de chasse:<b>\n")
        data = data.replace("Vitesse de chasse","\n<b>Vitesse de chasse:<b>\n")
        data = data.replace("Preuve","\n<b>Preuve:<b>\n")
        data = data.replace("🔊","")
...
function send_empty_data_link(){
    if(hasDLLink){
        dlws.send(JSON.stringify({"action":"GHOSTDATA","ghost":`None|<i>Cliquez sur un fantôme pour voir ses récits et ses comportements\n(Utilisez ' ù ' et ' * ' pour faire défiler les fantômes)<i>`}))
    }
}
...
function disconnect_link(reset=false,has_status=false){
    ...
    if(!reset){
        ...
        $("#link_id_disconnect").hide()
        if(!has_status){
            document.getElementById("link_id_note").innerText = "STATUT: Non lié"
